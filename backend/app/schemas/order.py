...
class OrderCreateRequest(BaseModel):
    file_ids: List[uuid.UUID] = Field(
        ...
        min_length=1,
        max_length=5,
        description="List of 1 to 5 uploaded file UUIDs",
    )
    print_side: PrintSideEnum = Field(
        ..., description="SINGLE_SIDE, DOUBLE_SIDE, or MULTI_PAGE"
    )
    color_mode: ColorModeEnum = Field(..., description="BW or COLOR")
    binding_type: BindingTypeEnum = Field(
        default=BindingTypeEnum.NONE,
        description="NONE, SPIRAL, SOFT_COVER, HARD_COVER, STAPLE_PINS",
    )
    copies: int = Field(
        default=1, ge=1, le=100, description="Number of print copies (1..100)"
    )


class OrderStatusUpdateRequest(BaseModel):
    status: OrderStatusEnum = Field(
        ..., description="Target advanced OrderStatusEnum state"
    )
    payment_method: Optional[PaymentMethodEnum] = Field(
        default=None, description="Required when marking status as PAID"
    )
...
class OrderResponse(BaseModel):
    ...
    def from_orm_order(cls, order) -> "OrderResponse":
        ...
        return cls(
            ...
            print_side=(
                order.print_side.value
                if hasattr(order.print_side, "value")
                else str(order.print_side)
            ),
            color_mode=(
                order.color_mode.value
                if hasattr(order.color_mode, "value")
                else str(order.color_mode)
            ),
            binding_type=(
                order.binding_type.value
                if hasattr(order.binding_type, "value")
                else str(order.binding_type)
            ),
            copies=order.copies,
