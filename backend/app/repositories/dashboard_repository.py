...
class DashboardRepository:
    def get_dashboard_stats(self, db: Session) -> Dict:
        ...
        pending_orders = (
            db.query(func.count(Order.id))
            .filter(Order.status == OrderStatusEnum.PENDING_PAYMENT)
            .scalar()
            or 0
        )
        printing_orders = (
            db.query(func.count(Order.id))
            .filter(Order.status == OrderStatusEnum.PRINTING)
            .scalar()
            or 0
        )
        ready_orders = (
            db.query(func.count(Order.id))
            .filter(Order.status == OrderStatusEnum.READY_FOR_PICKUP)
            .scalar()
            or 0
        )
        completed_orders = (
        ...
        today_revenue = (
            db.query(func.sum(LedgerEntry.amount))
            .filter(LedgerEntry.amount > 0, LedgerEntry.created_at >= start_of_day)
            .scalar()
            or 0.0
        )
        monthly_revenue = (
            db.query(func.sum(LedgerEntry.amount))
            .filter(LedgerEntry.amount > 0, LedgerEntry.created_at >= start_of_month)
            .scalar()
            or 0.0
        )
        ...
        cash_balance = (
            db.query(func.sum(LedgerEntry.amount))
            .filter(LedgerEntry.entry_type.like("%_CASH"))
            .scalar()
            or 0.0
        )
        ...
        active_students = (
            db.query(func.count(func.distinct(Order.student_id))).scalar() or 0
        )

        # Files and Pages
        total_files = db.query(func.count(OrderFile.id)).scalar() or 0
        total_pages = (
            db.query(func.sum(Order.page_count * Order.copies))
            .filter(Order.status == OrderStatusEnum.COMPLETED)
            .scalar()
            or 0
        )
        ...
        low_stock_count = (
            ...
            .filter(InventoryItem.current_stock <= InventoryItem.low_stock_threshold)
            .scalar()
            or 0
        )
